...
import { FiMail, FiPhone, FiUser, FiMessageSquare, FiSend, FiGlobe, FiHeart, FiLayers } from 'react-icons/fi';
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#030712] transition-colors duration-500 overflow-hidden relative pt-24 pb-12 px-4">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          >
            Let's Make a <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-emerald-400">Difference.</span>
          </motion.h1>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-12 items-start">
          ...
          >
            <div className="bg-white/50 dark:bg-gray-900/50 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/20 dark:border-gray-800 shadow-2xl">
              ...
              <div className="space-y-8">
                <ContactInfoItem icon={<FiPhone />} title="Call Us" detail="+1 (234) 567-890" />
                <ContactInfoItem icon={<FiMail />} title="Email Us" detail="hello@ngo-foundation.org" />
                <ContactInfoItem icon={<FiMessageSquare />} title="Visit Us" detail="123 Hope Street, Unity City" />
              </div>
            </div>
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6 mt-6">
              <InputField label="Email" placeholder="john@example.com" icon={<FiMail />} type="email" />
              <InputField label="Contact Number" placeholder="+1 000 000 000" icon={<FiPhone />} type="tel" />
            </div>

            <div className="mt-6">
              <InputField label="Subject" placeholder="How can we help?" icon={<FiLayers />} />
            </div>

            <div className="mt-6">
              <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">Your Message</label>
...
const InputField = ({ label, placeholder, icon, type = "text" }) => (
  <div className="w-full">
    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">{label}</label>
    <div className="relative group">
...
const ContactInfoItem = ({ icon, title, detail }) => (
  <div className="flex items-center gap-5 group">
    ...
    <div>
      <h4 className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-black mb-1">{title}</h4>
      <p className="text-gray-900 dark:text-gray-200 font-bold text-lg">{detail}</p>
    </div>
  </div>
);
